...
query extraction, and anti-bot mitigation.
...
class FlipkartScraper(BaseScraper):
    """
    Scrapes product search results and product pages from Flipkart.com.
    """
    ...
    def search(self, query: str, max_pages: int = 1, max_items: int = 20) -> list[dict[str, Any]]:
        """
        Search Flipkart for a query keyword and scrape matching products.
        ...
        for page in range(1, max_pages + 1):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.logger.warning("Flipkart returned status %d. Applying intelligent fallback.", response.status_code)
                    break

            except Exception as e:
                self.logger.error("Error scraping Flipkart search page %d: %s", page, e)
                break
    ...
    def _extract_search_results(self, soup: BeautifulSoup, query: str) -> list[dict[str, Any]]:
        """Extract product items from Flipkart search results page."""
        products = []

        cards = soup.select("div[data-id]")
        if not cards:
            cards = soup.select("div._1AtVbE, div._75nlfW, div.slAVV4, div._2kHMtA, div._13oc-S")

        for card in cards:
            fsn = card.get("data-id", "").strip()

            # Product Title
            title_tag = card.select_one("div.KzDlHZ, div._4rR01T, a.wjcEIp, a.IRpwTa, div._2W9c47, a[title]")
            title = ""
            if title_tag:
            ...
            # Price
            price_tag = card.select_one("div.Nx9bqj, div._30jeq3")
            price_str = price_tag.get_text(strip=True).replace("₹", "").replace(",", "") if price_tag else ""
            ...
            # Rating
            rating_tag = card.select_one("div.XQDdHH, div._3LWZlK")
            rating_str = rating_tag.get_text(strip=True) if rating_tag else ""
            try:
            ...
            # Product URL
            link_tag = card.select_one("a[href*='/p/']") or card.select_one("a[href]")
            href = link_tag.get("href", "") if link_tag else ""
            product_url = f"{FLIPKART_BASE_URL}{href}" if href.startswith("/") else href

            # Image
            img_tag = card.select_one("img.DByuf4, img._396cs4")
    ...
    def _extract_product(self, soup: BeautifulSoup, url: str) -> dict[str, Any]:
        ...
        title_el = soup.select_one("span.B_NuCI, h1.ProductTitle")
        title = title_el.get_text(strip=True) if title_el else "Unknown Flipkart Product"

        price_el = soup.select_one("div.Nx9bqj, div._30jeq3")
